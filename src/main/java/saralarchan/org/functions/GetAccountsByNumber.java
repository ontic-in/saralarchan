...
public class GetAccountsByNumber extends BaseRequestProcessor {
	...
	private HashMap postTransform(HashMap response) {
		...
		for (int i = 0; i < linesArray.length(); i++) {
			JSONObject line = linesArray.getJSONObject(i);
			String provisioningStatus = line.getString("provisioningStatus") != null
					? line.getString("provisioningStatus")
					: "";
			provisioningStatusList.add(provisioningStatus.toLowerCase());
		}
		boolean areAllLinesTerminated = (provisioningStatusList.contains("active")
				|| provisioningStatusList.contains("suspended")) ? false : true;
		...
		if (isPrim) {
			...
			primaryEmailAddress = primaryEmailAddress != null
					? (primaryEmailAddress.contains("@dummy.m1.com.sg") ? null : primaryEmailAddress)
					: null;
		...
		if (isPrim) {
			...
			for (int i = 0; i < linesArray.length(); i++) {
				...
				String hasSimActivate = line.getString("hasSIMActivated") != null
						? line.getString("hasSIMActivated").toUpperCase()
						: "";

				// String migrationDateString = line.getString("migrationDate") != null ?
				// line.getString("migrationDate") : "";
